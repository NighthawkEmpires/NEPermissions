...
public class PromoteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                player.sendMessage(Lang.CHAT_TAG.getServerChatTag() + ChatColor.AQUA + "/promote " + ChatColor.DARK_AQUA + "[player]");
                return true;
            } else if (args.length == 1) {
                Bukkit.dispatchCommand(player, "groups promote " + args[0]);
                return true;
            }
        }
        return true;
